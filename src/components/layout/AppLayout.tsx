          ...
          <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Tool categories">
            {TOOL_CATEGORIES.map((category) => (
              <div key={category.id} className="group relative">
                <span className="cursor-default rounded-lg px-2.5 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900">
                  {category.title}
                </span>
                <div className="invisible absolute left-0 top-full z-20 min-w-56 translate-y-1 rounded-xl border border-slate-200 bg-white py-2 opacity-0 shadow-xl transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
      ...
      <footer className="border-t border-slate-200 bg-white/90">
        <PageContainer width="full" className="py-5">
          <p className="text-center text-sm text-slate-500 sm:text-left">
            {SITE_NAME} — {TOOL_CATEGORIES.reduce((n, c) => n + c.tools.length, 0)} free tools.
